...
def printStates(codeType, mData):
	if ((int(mData[0]) >= 0 and int(mData[0]) <= 255) and (int(mData[1]) >= 1 and int(mData[1]) <= 4) and (int(mData[2]) >= 0 and int(mData[2]) <= 1) and ((len(mData) == 4 and int(mData[3]) >= 0 and int(mData[3]) <= 1) or len(mData) == 3)):
		...
		if (int(mData[1]) == 1):
			print("Cihaz: " + mData[1] + " - Kombi")
		elif (int(mData[1]) == 2):
			print("Cihaz: " + mData[1] + " - Camasir Makinesi")
		elif (int(mData[1]) == 3):
			print("Cihaz: " + mData[1] + " - Ütü")
		elif (int(mData[1]) == 4):
			print("Cihaz: " + mData[1] + " - Elekrikli Süpürge")
		else:
	...
	if (len(i) > 0):
		...
		if (codeType == "send"):
			if (len(msgChunk) != 4):
				...
				printStates(codeType + " - Giden", msgChunk)
		elif (codeType == "receive"):
			if (len(msgChunk) != 3):
				print("Error: ikinci bolum hatali")
			else:
				printStates(codeType + " - Gelen", msgChunk)
		else:
